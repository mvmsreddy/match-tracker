...
function reasonLabel(pt, selfName, oppName) {
  ...
  if (pt.reason === 'Winner') return name + ' ' + pt.stroke.toLowerCase() + (pt.stroke === 'Serve' ? ' (ace)' : '') + ' winner' + (pt.isReturn ? ' (return)' : '') + firstFaultNote + infractionNote;
  const kind = pt.reason === 'ForcedError' ? 'forced error' : 'unforced error';
  const locSuffix = pt.location ? ' - ' + pt.location : '';
  return name + ' ' + pt.stroke.toLowerCase() + ' ' + kind + locSuffix + (pt.isReturn ? ' (return)' : '') + firstFaultNote + infractionNote;
}
