...
// Parent ↔ player link management — same request/accept/unlink flow as
// CoachPlayersPage.jsx (src/pages/CoachPlayersPage.jsx), against the
// separate parent_player_links table (Phase 33) instead of
// coach_player_links. Kept as a sibling page rather than folding into
// CoachPlayersPage to avoid threading a third relationship type through
// that component's isCoach-branched logic.
export default function ParentPlayersPage() {
  ...
  // Pending incoming (player sees requests from parents)
  const incomingPending = pendingLinks.filter(l => l.playerId === user.id);
  // Pending outgoing (parent sees requests they sent)
  const outgoingPending = pendingLinks.filter(l => l.parentId === user.id);
  ...
  async function handleSendRequest(playerId) {
    setActionError('');
    try {
      const link = await api.sendParentRequest(user.id, playerId);
  ...
  async function handleRespond(linkId, status) {
    setActionError('');
    try {
      const updated = await api.respondToParentRequest(linkId, status);
  ...
  function getOtherParty(link) {
    return isParent ? link.player : link.parent;
  }

  return (
    <div className="px-4 lg:px-8 py-6 lg:py-8 max-w-3xl mx-auto space-y-4">
      <div>
        <h1 className="font-display font-extrabold text-2xl tracking-tighter">{isParent ? 'My Players' : 'My Parents'}</h1>
        <div className="text-sm text-muted-foreground mt-0.5">
          {isParent ? 'Players linked to your parent profile (read-only)' : 'Parents connected to your player profile'}
        </div>
      </div>
      ...
      {links !== null && activeLinks.length === 0 && pendingLinks.length === 0 && (
        <div className="border border-dashed border-border rounded-sm p-6 text-center text-sm text-muted-foreground">
          {isParent
            ? 'No players linked yet. Search above to find and connect with your child.'
            : 'No parents linked yet. Ask your parent to send you a connection request.'}
        </div>
      )}
      ...
      {activeLinks.length > 0 && (
        <Card className="p-4 sm:p-6">
          <div className="text-xs uppercase tracking-wider font-bold text-muted-foreground mb-3">
            {isParent ? `Linked Players (${activeLinks.length})` : `My Parents (${activeLinks.length})`}
          </div>
          <div className="space-y-2">
