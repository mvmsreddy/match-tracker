...
export function generateOOPPdf({ week, matches }) {
  ...
  for (const day of days) {
    ...
    const refParts = [week.referee, week.refereePhone].filter(Boolean);
    const leftFooter = refParts.length ? `AITA Referee: ${refParts.join(' · ')}` : 'Generated by Match Tracker';
    doc.text(leftFooter, margin, pageH - 4);
